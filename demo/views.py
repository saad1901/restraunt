...
def owner(request):
    ...
    orders = Order.objects.filter(completed=False, hotel=2)
...
def complete_order(request):
    try:
        ...
        order = Order.objects.get(id=order_id, completed=False, hotel=2)
...
def delete_order(request):
    try:
        ...
        if not Order.objects.filter(table=table, completed=False, hotel=2).exists():
            table.occupied = False
            table.save()
...
def add_table(request):
    ...
    if request.method == "POST":
        ...
        if name:
            Table.objects.create(name=name, occupied=False, hotel=2)  # Always False when adding a new table
...
def dailytransc(request):
    ...
    if selected_date:
        try:
            ...
            orders = Order.objects.filter(hotel=2).prefetch_related('orderitems').order_by('created_at')
...
def timeanalysis(request):
    ...
    orders = (
        Order.objects
        .filter(created_at__gte=start_date, hotel=2)
    ...
    most_ordered_items = (
        OrderItems.objects
        .filter(order__created_at__gte=start_date, order__hotel=2, order__completed=True)
        .values('item__name')
...
def bill_history(request):
    records = PaymentRecord.objects.filter(hotel=2).order_by('-created_at')
    context = {
        'records' : records
    }
